...
const AspectRatioSelector = ({value, onChange} : {
    value: AspectRatio;onChange: (ratio: AspectRatio) => void}) => {
  ...
  return (
    <div className='space-y-3'>
      ...
      <div className='flex flex-wrap gap-2'>
        ...
        return (
            <button key={ratio} type='button' onClick={() => onChange(ratio)} className={`flex items-center gap-2 px-5 py-2.5 rounded-md text-sm  border transition border-white/10 ${selected ? ' bg-white/10' : 'hover:bg-white/6'}`}>
                {icomMap[ratio]}
